package com.mock.pay.zx.qq.impl;
...
@Service
public class QQScanServiceImpl implements QQScanService{

    private static final Logger logger = LoggerFactory.getLogger(QQScanServiceImpl.class);
...
//
                ...
                respMap.put("attach", map.get("attach"));// 商家数据包，原样返回
                respMap.put("bank_type", map.get("bank_type"));// 银行类型
                respMap.put("bank_billno", map.get("bank_billno"));// 银行订单号，若为手Q支付则为空
