...
@Service
public class AlipayScanServiceImpl implements AlipayScanService {

    private static final Logger logger = LoggerFactory.getLogger(AlipayScanServiceImpl.class);
    ...
    public void payResp(HttpServletResponse resp, Map<String, String> map) throws IOException {
        ...
        if ("1".equals(dd.get("key_iswork"))) {
            Signkey = ZXGetValue.getKeyWithmichId(mch_id);
        } else {
        ...
        if ("0".equals(status)) {
            ...
            if ("0".equals(result_code)) {
                ...
                respMap.put("code_url",
                        "http://192.168.74.163:2225/qrcode?uuid=" + map.get(service) + StringUtil.getRandomString(8)); // 二维码链接
                respMap.put("code_img_url",
                        "http://192.168.74.163:2225/qrcode?uuid=" + map.get(service) + StringUtil.getRandomString(8)); // 二维码图片
            }
            ...
            String sign = MD5.sign(preStr, "&key=" + Signkey, "utf-8");// gbk
            // 验理证
            // //默认要保证一致utf-8
    ...
    public  void payNotify(Map<String, String> map) throws IOException, InterruptedException {
        ...
        if ("0".equals(payNotify_status)) {// 以下字段在 status 为 0的时候有返回
            ...
            if ("0".equals(payNotify_result_code)) {
                ...
                respMap.put("trade_type", map.get("service")); // 交易类型----非必传
                // map.get("service")
        ...
        logger.debug("支付宝扫码支付签名：" + Signkey);
