...
import com.mock.pay.pf.PFAlipayScanService;
import com.mock.sign.Base64;
import com.mock.sign.ChinaCardPosUtil;

import com.mock.sign.KeyString;
import com.mock.util.MapUtil;
...
@Service
public class PFAlipayScanServiceImpl implements PFAlipayScanService{
    private static final Logger logger = LoggerFactory.getLogger(PFAlipayScanServiceImpl.class);
...
//            redisCache.put(out_trade_no, tmpString);
        ...
        if("0000".equals(respCode)){
            String url="https://qr.alipay.com/"+ map.get(productId) + StringUtil.getRandomString(8);
            String codeUrl= Base64.encode(url.getBytes());
            respMap.put("codeUrl",codeUrl);
            respMap.put("imgUrl",codeUrl);
...
//        String po= MapUtil.transMapToString(respMap);
//        logger.info("Map转String"+po);
//        String pom=po.substring(1,po.length()-1);
//        logger.info("去掉{}后参数"+pom);
